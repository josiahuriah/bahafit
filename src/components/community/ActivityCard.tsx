...
/**
 * Sanity's image CDN can resize/compress on the fly via URL params.
 * Feed thumbnails don't need full-resolution originals — this cuts
 * media bandwidth (the main hosting cost driver) dramatically.
 */
function optimized(url: string, width: number): string {
  if (!url.startsWith('https://cdn.sanity.io/images/')) return url
  return `${url}${url.includes('?') ? '&' : '?'}auto=format&w=${width}&q=75`
}
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      ...
      {activity.media && activity.media.length > 0 && (
        detailed ? (
          <div className={`px-4 sm:px-5 pb-2 grid gap-2 ${activity.media.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
            {activity.media.map((m, i) =>
              m.type === 'image' ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img key={i} src={optimized(m.url, 1200)} alt={activity.title} className="w-full rounded-xl object-cover max-h-96" />
              ) : (
                <video key={i} src={m.url} controls playsInline preload="metadata" className="w-full rounded-xl max-h-96 bg-black" />
              )
            )}
          </div>
        ) : (
          <Link href={`/community/activity/${activity._id}`} className="block px-4 sm:px-5 pb-2 relative">
            {activity.media[0].type === 'image' ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img
                src={optimized(activity.media[0].url, 800)}
                alt={activity.title}
              ...
              <div className="relative">
                <video src={activity.media[0].url} muted playsInline preload="metadata" className="w-full rounded-xl object-cover h-56 bg-black" />
            ...
            {activity.media.length > 1 && (
              <span className="absolute top-2 right-7 bg-black/60 text-white text-xs px-2 py-0.5 rounded-full">
