...
// Pull the city portion out of a seed event's formatted "City, Bahamas"
// location string so the selector can fall back to seeded content.
function cityFromSeedLocation(location: string): string {
 ...
 * while preserving the first-seen display spelling. When there are no real
 * upcoming events yet, we fall back to the seeded cities — matching how the
 * rest of the homepage pads Featured Events/Listings with seed content.
 */
export async function getUpcomingCities(): Promise<string[]> {
  ...
  const cities = dedupeCities([...user, ...sanity])
  if (cities.length) return cities
  return dedupeCities(seedEvents.map((e) => cityFromSeedLocation(e.location)))
}
